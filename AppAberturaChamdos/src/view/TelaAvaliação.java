/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/GUIForms/JInternalFrame.java to edit this template
 */
package view;

import javax.swing.JOptionPane;

/**
 *
 * @author diegu
 */
public class TelaAvaliação extends javax.swing.JInternalFrame {

    /**
     * Creates new form TelaAvaliação1
     */
    public TelaAvaliação() {
        initComponents();
    }

    /**
     * This method is called from within the constructor to initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is always
     * regenerated by the Form Editor.
     */
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        txtSenhaChamado = new javax.swing.JPasswordField();
        txtPorqueDaClassificação = new javax.swing.JTextField();
        BtnMandarAvaliação = new javax.swing.JButton();
        txtNomeChamado = new javax.swing.JTextField();
        txtClassificação = new javax.swing.JTextField();
        txtAlgumaMelhoria = new javax.swing.JTextField();
        jLabel1 = new javax.swing.JLabel();

        setClosable(true);
        setTitle("Tela de Avalição");
        getContentPane().setLayout(new org.netbeans.lib.awtextra.AbsoluteLayout());

        txtSenhaChamado.setBorder(javax.swing.BorderFactory.createEtchedBorder());
        getContentPane().add(txtSenhaChamado, new org.netbeans.lib.awtextra.AbsoluteConstraints(170, 510, 260, 40));

        txtPorqueDaClassificação.setFont(new java.awt.Font("Gill Sans MT", 0, 14)); // NOI18N
        txtPorqueDaClassificação.setBorder(javax.swing.BorderFactory.createEtchedBorder());
        getContentPane().add(txtPorqueDaClassificação, new org.netbeans.lib.awtextra.AbsoluteConstraints(170, 290, 490, 60));

        BtnMandarAvaliação.setBackground(new java.awt.Color(102, 153, 255));
        BtnMandarAvaliação.setFont(new java.awt.Font("Gill Sans MT", 0, 36)); // NOI18N
        BtnMandarAvaliação.setForeground(new java.awt.Color(255, 255, 255));
        BtnMandarAvaliação.setBorder(null);
        BtnMandarAvaliação.setContentAreaFilled(false);
        BtnMandarAvaliação.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));
        BtnMandarAvaliação.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                BtnMandarAvaliaçãoActionPerformed(evt);
            }
        });
        getContentPane().add(BtnMandarAvaliação, new org.netbeans.lib.awtextra.AbsoluteConstraints(460, 520, 240, 70));

        txtNomeChamado.setFont(new java.awt.Font("Gill Sans MT", 0, 14)); // NOI18N
        txtNomeChamado.setBorder(javax.swing.BorderFactory.createEtchedBorder());
        getContentPane().add(txtNomeChamado, new org.netbeans.lib.awtextra.AbsoluteConstraints(170, 160, 490, 30));

        txtClassificação.setFont(new java.awt.Font("Gill Sans MT", 0, 14)); // NOI18N
        txtClassificação.setBorder(javax.swing.BorderFactory.createEtchedBorder());
        getContentPane().add(txtClassificação, new org.netbeans.lib.awtextra.AbsoluteConstraints(170, 220, 490, 30));

        txtAlgumaMelhoria.setFont(new java.awt.Font("Gill Sans MT", 0, 14)); // NOI18N
        txtAlgumaMelhoria.setBorder(javax.swing.BorderFactory.createEtchedBorder());
        getContentPane().add(txtAlgumaMelhoria, new org.netbeans.lib.awtextra.AbsoluteConstraints(170, 390, 490, 80));

        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Images/TelaAvaliação_1.png"))); // NOI18N
        jLabel1.setText("jLabel1");
        getContentPane().add(jLabel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, 800, 600));

        pack();
    }// </editor-fold>//GEN-END:initComponents

    public boolean checkLogin(String login, String senha){
        return login.equals("Diego") && senha.equals("123");
    }
    
    public boolean checkLogin1(String login, String senha){
        return login.equals("Carol") && senha.equals("123");
    }
    
    public boolean checkLogin2(String login, String senha){
        return login.equals("Leonardo") && senha.equals("123");
    }
    
    public boolean checkLogin3(String login, String senha){
        return login.equals("Arthur") && senha.equals("123");
    }
    
    public boolean checkLogin4(String login, String senha){
        return login.equals("Vinicius") && senha.equals("123");
    }
    
    public boolean checkLogin5(String login, String senha){
        return login.equals("Bruno") && senha.equals("123");
    }
    
    public boolean checkLogin6(String login, String senha){
        return login.equals("Gustavo") && senha.equals("123");
    }
    
    private void BtnMandarAvaliaçãoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnMandarAvaliaçãoActionPerformed

        if(this.checkLogin(txtNomeChamado.getText(), new String(txtSenhaChamado.getPassword()))){
            JOptionPane.showMessageDialog(null, "Avaliação enviada com sucesso!");
            this.setVisible(false);

        }else if((this.checkLogin1(txtNomeChamado.getText(), new String(txtSenhaChamado.getPassword())))){
            JOptionPane.showMessageDialog(null, "Avaliação enviada com sucesso!");
            this.setVisible(false);

        }else if((this.checkLogin2(txtNomeChamado.getText(), new String(txtSenhaChamado.getPassword())))){
            JOptionPane.showMessageDialog(null, "Avaliação enviada com sucesso!");
            this.setVisible(false);

        }else if((this.checkLogin3(txtNomeChamado.getText(), new String(txtSenhaChamado.getPassword())))){
            JOptionPane.showMessageDialog(null, "Avaliação enviada com sucesso!");
            this.setVisible(false);

        }else if((this.checkLogin4(txtNomeChamado.getText(), new String(txtSenhaChamado.getPassword())))){
            JOptionPane.showMessageDialog(null, "Avaliação enviada com sucesso!");
            this.setVisible(false);

        }else if((this.checkLogin5(txtNomeChamado.getText(), new String(txtSenhaChamado.getPassword())))){
            JOptionPane.showMessageDialog(null, "Avaliação enviada com sucesso!");
            this.setVisible(false);

        }else if((this.checkLogin6(txtNomeChamado.getText(), new String(txtSenhaChamado.getPassword())))){
            JOptionPane.showMessageDialog(null, "Avaliação enviada com sucesso!");
            this.setVisible(false);

        }else{
            JOptionPane.showMessageDialog(null, "Senha ou Usuário incorreta.");
        }
    }//GEN-LAST:event_BtnMandarAvaliaçãoActionPerformed


    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javax.swing.JButton BtnMandarAvaliação;
    private javax.swing.JLabel jLabel1;
    private javax.swing.JTextField txtAlgumaMelhoria;
    private javax.swing.JTextField txtClassificação;
    private javax.swing.JTextField txtNomeChamado;
    private javax.swing.JTextField txtPorqueDaClassificação;
    private javax.swing.JPasswordField txtSenhaChamado;
    // End of variables declaration//GEN-END:variables
}
